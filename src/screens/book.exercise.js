...
const getBookById = (bookId, user) => async () => {
  return await client(`books/${bookId}`, {token: user.token})
}

const getListItems = user => async () => {
  const result = await client('list-items', {token: user.token})
  return result.listItems
}

const updateListItem = (data, user) => {
...
function BookScreen({user}) {
  ...
  const {data: listItems} = useQuery('list-items', getListItems(user))
  const listItem =
...
function NotesTextarea({listItem, user}) {
  const [mutate] = useMutation(data => updateListItem(data, user), {
